...
export default function ProvisionPage() {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-demo-bluePale px-4">
      <section className="w-full max-w-md rounded-[28px] bg-white p-8 text-center shadow-soft">
        <BrandLogo />
        <div className="mx-auto mt-8 flex h-14 w-14 items-center justify-center rounded-full bg-demo-bluePale">
          {status === 'loading' ? (
            <span className="h-7 w-7 animate-spin rounded-full border-4 border-demo-sky border-t-demo-blue" />
          ) : (
            <span className="text-2xl font-extrabold text-demo-blue">{status === 'success' ? '✓' : '!'}</span>
          )}
        </div>
        <h1 className="mt-6 text-2xl font-extrabold text-demo-navy">Microsite personalizado</h1>
